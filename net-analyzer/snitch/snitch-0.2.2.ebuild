...
# TODO: Move to a vendor tarball.
# For now, just don't want to maintain a vendor tarball.
_EGO_SUM=(
...
src_compile() {
	...
	local git_commit="v${PV}"
	local build_date=$(date -u +%Y-%m-%dT%H:%M:%SZ)

	ego build -o snitch \
		-ldflags "-X github.com/karol-broda/snitch/cmd.Version=${PV} -X github.com/karol-broda/snitch/cmd.Commit=${git_commit} -X github.com/karol-broda/snitch/cmd.Date=${build_date}" \
		.
}
